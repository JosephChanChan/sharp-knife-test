...
public class SingleHead {
    ...
    public void test() {
        ...
        ScheduleRequest<SimpleContext, SimpleResult> request = new ScheduleRequest<>(singleTaskType, context, result);

        ExecutionResult executionResult = concurrentScheduler.scheduleSyn(request);
        if (executionResult.hasError()) {
            throw new RuntimeException("SingleHead execute error", executionResult.getError());
        }
        log.info("SingleHead done, res={}", result);
    }
}
