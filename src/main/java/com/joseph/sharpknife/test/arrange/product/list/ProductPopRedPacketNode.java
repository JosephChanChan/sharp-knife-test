...
@TaskConfig(taskName = "productPopRedPacketNode", taskType = "productListType", timeout = 100)
public class ProductPopRedPacketNode implements TaskNode<ProductListContext, ProductListResult> {
    ...
    public BiConsumer<ProductListContext, ProductListResult> getTask() {
        return (context, result) -> {
            ...
            log.info("productPopRedPacketNode task will cost={}", millis);
            ThreadKit.sleep(millis);
            result.finishNode("productPopRedPacketNode", result.getOrder().getAndIncrement(),
                    new String[]{
                            "productIdAfterNode",
