...
@Slf4j
@Component
@DependsOn({"productInfoNode", "productModelNode", "productBrandNode", "productParamNode"})
@TaskConfig(taskName = "productTitleNode", taskType = "productListType", timeout = 100)
public class ProductTitleNode implements TaskNode<ProductListContext, ProductListResult> {
    ...
    public BiConsumer<ProductListContext, ProductListResult> getTask() {
        return (context, result) -> {
            ...
            log.info("productTitleNode task will cost={}", millis);
            ThreadKit.sleep(millis);
            result.finishNode("productTitleNode", result.getOrder().getAndIncrement(),
                    new String[]{
                            "productInfoNode",
                            "productModelNode",
                            "productBrandNode",
