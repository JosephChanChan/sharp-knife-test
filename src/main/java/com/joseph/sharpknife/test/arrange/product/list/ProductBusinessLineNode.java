...
@TaskConfig(taskName = "productBusinessLineNode", taskType = "productListType", timeout = 100)
public class ProductBusinessLineNode implements TaskNode<ProductListContext, ProductListResult> {
    ...
    public BiConsumer<ProductListContext, ProductListResult> getTask() {
        return (context, result) -> {
            ...
            log.info("productBusinessLineNode task will cost={}", millis);
            ThreadKit.sleep(millis);
            result.finishNode("productBusinessLineNode", result.getOrder().getAndIncrement(),
                    new String[]{
                            "productIdAfterNode"
                    });
        };
