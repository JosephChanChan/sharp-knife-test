...
@TaskConfig(taskName = "productPriceFlowNode", taskType = "productListType", timeout = 100)
public class ProductPriceFlowNode implements TaskNode<ProductListContext, ProductListResult> {
    ...
    public BiConsumer<ProductListContext, ProductListResult> getTask() {
        return (context, result) -> {
            ...
            log.info("productPriceFlowNode task will cost={}", millis);
            ThreadKit.sleep(millis);
            result.finishNode("productPriceFlowNode", result.getOrder().getAndIncrement(),
                    new String[]{
                            "productIdAfterNode"
                    });
        };
