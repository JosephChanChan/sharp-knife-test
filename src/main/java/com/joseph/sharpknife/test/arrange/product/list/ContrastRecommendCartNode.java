...
@TaskConfig(taskName = "contrastRecommendCartNode", taskType = "productListType", timeout = 200)
public class ContrastRecommendCartNode implements TaskNode<ProductListContext, ProductListResult> {
    ...
    public BiConsumer<ProductListContext, ProductListResult> getTask() {
        return (context, result) -> {
            int millis = ThreadKit.boundMillis(220);
            log.info("contrastRecommendCartNode task will cost={}", millis);
            ThreadKit.sleep(millis);
            result.finishNode("contrastRecommendCartNode", result.getOrder().getAndIncrement(), new String[]{"productIdByContrastRecommendNode"});
        };
    }

    @Override
    public void onSuccess(ProductListContext executionContext, ProductListResult executionResult) {
        log.info("contrastRecommendCartNode done");
    }

    @Override
    public void onError(ProductListContext executionContext, ProductListResult executionResult, Exception e) {
        log.error("contrastRecommendCartNode error", e);
    }
}
