...
@Slf4j
@Component
@DependsOn("productIdByContrastRecommendNode")
@TaskConfig(taskName = "contrastRecommendFavouriteNode", taskType = "productListType", timeout = 200)
public class ContrastRecommendFavouriteNode implements TaskNode<ProductListContext, ProductListResult> {
    ...
    public BiConsumer<ProductListContext, ProductListResult> getTask() {
        return (context, result) -> {
            int millis = ThreadKit.boundMillis(205);
            log.info("contrastRecommendFavouriteNode task will cost={}", millis);
            ThreadKit.sleep(millis);
            result.finishNode("contrastRecommendFavouriteNode", result.getOrder().getAndIncrement(), new String[]{"productIdByContrastRecommendNode"});
        };
    }

    @Override
    public void onSuccess(ProductListContext executionContext, ProductListResult executionResult) {
        log.info("contrastRecommendFavouriteNode done");
    }

    @Override
    public void onError(ProductListContext executionContext, ProductListResult executionResult, Exception e) {
        log.error("contrastRecommendFavouriteNode error", e);
    }
}
