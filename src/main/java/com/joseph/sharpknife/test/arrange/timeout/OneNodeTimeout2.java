...
@Slf4j
@Component
@TaskConfig(taskName = "oneNodeTimeout2", taskType = "oneNodeTimeoutType")
public class OneNodeTimeout2 implements TaskNode<SimpleContext, Integer> {


    @Override
    public BiPredicate<SimpleContext, Integer> getPrediction() {
        return (context, result) -> {
            log.info("OneNodeTimeout2 prediction true");
            return true;
        };
    }
    ...
    public BiConsumer<SimpleContext, Integer> getTask() {
        return (context, result) -> {
            ...
            log.info("OneNodeTimeout2 task will cost={}", millis);
