...
@Slf4j
@Component
@TaskConfig(taskName = "productIdByContrastRecommendNode", taskType = "productListType", timeout = 200)
public class ProductIdByContrastRecommendNode implements TaskNode<ProductListContext, ProductListResult> {
    ...
    public BiConsumer<ProductListContext, ProductListResult> getTask() {
        return (context, result) -> {
            ...
            log.info("productIdByContrastRecommendNode task will cost={}", millis);
            ThreadKit.sleep(millis);
            result.finishNode("productIdByContrastRecommendNode", result.getOrder().getAndIncrement(), null);
        };
    }

    @Override
    public void onSuccess(ProductListContext executionContext, ProductListResult executionResult) {
        log.info("productIdByContrastRecommendNode done");
    }

    @Override
    public void onError(ProductListContext executionContext, ProductListResult executionResult, Exception e) {
        log.error("productIdByContrastRecommendNode error", e);
    }
}
