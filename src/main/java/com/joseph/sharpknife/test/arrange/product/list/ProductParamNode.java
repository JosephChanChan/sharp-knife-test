...
@TaskConfig(taskName = "productParamNode", taskType = "productListType", timeout = 100)
public class ProductParamNode implements TaskNode<ProductListContext, ProductListResult> {
    ...
    public BiConsumer<ProductListContext, ProductListResult> getTask() {
        return (context, result) -> {
            ...
            log.info("productParamNode task will cost={}", millis);
            ThreadKit.sleep(millis);
            result.finishNode("productParamNode", result.getOrder().getAndIncrement(),
                    new String[]{
                            "productExtInfoNode"
                    });
        };
