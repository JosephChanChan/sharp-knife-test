...
@Slf4j
@Component
@TaskConfig(taskName = "userDefaultReceiveAddressCityInfoNode", taskType = "productListType")
public class UserDefaultReceiveAddressCityInfoNode implements TaskNode<ProductListContext, ProductListResult> {
    ...
    public BiConsumer<ProductListContext, ProductListResult> getTask() {
        return (context, result) -> {
            ...
            log.info("userDefaultReceiveAddressCityInfoNode task will cost={}", millis);
            ThreadKit.sleep(millis);
            result.finishNode("userDefaultReceiveAddressCityInfoNode", result.getOrder().getAndIncrement(), null);
        };
    }

    @Override
    public void onSuccess(ProductListContext executionContext, ProductListResult executionResult) {
        log.info("userDefaultReceiveAddressCityInfoNode done");
    }

    @Override
    public void onError(ProductListContext executionContext, ProductListResult executionResult, Exception e) {
        log.error("userDefaultReceiveAddressCityInfoNode error", e);
    }
}
