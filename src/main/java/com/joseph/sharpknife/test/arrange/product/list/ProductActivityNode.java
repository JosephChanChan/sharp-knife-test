...
@TaskConfig(taskName = "productActivityNode", taskType = "productListType", timeout = 100)
public class ProductActivityNode implements TaskNode<ProductListContext, ProductListResult> {
    ...
    public BiConsumer<ProductListContext, ProductListResult> getTask() {
        return (context, result) -> {
            ...
            log.info("productActivityNode task will cost={}", millis);
            ThreadKit.sleep(millis);
            result.finishNode("productActivityNode", result.getOrder().getAndIncrement(),
                    new String[]{
                            "productIdAfterNode"
                    });
        };
