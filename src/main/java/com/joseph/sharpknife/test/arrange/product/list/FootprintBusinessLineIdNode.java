...
@TaskConfig(taskName = "footprintBusinessLineIdNode", taskType = "productListType", timeout = 200)
public class FootprintBusinessLineIdNode implements TaskNode<ProductListContext, ProductListResult> {
    ...
    public BiConsumer<ProductListContext, ProductListResult> getTask() {
        return (context, result) -> {
            ...
            log.info("footprintBusinessLineIdNode task will cost={}", millis);
            ThreadKit.sleep(millis);
            result.finishNode("footprintBusinessLineIdNode", result.getOrder().getAndIncrement(), new String[]{"productIdByFootprintNode"});
        };
    }

    @Override
    public void onSuccess(ProductListContext executionContext, ProductListResult executionResult) {
        log.info("footprintBusinessLineIdNode done");
    }

    @Override
    public void onError(ProductListContext executionContext, ProductListResult executionResult, Exception e) {
        log.error("footprintBusinessLineIdNode error", e);
    }
}
