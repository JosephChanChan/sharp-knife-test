...
@TaskConfig(taskName = "productBrandNode", taskType = "productListType", timeout = 100)
public class ProductBrandNode implements TaskNode<ProductListContext, ProductListResult> {
    ...
    public BiConsumer<ProductListContext, ProductListResult> getTask() {
        return (context, result) -> {
            ...
            log.info("productBrandNode task will cost={}", millis);
            ThreadKit.sleep(millis);
            result.finishNode("productBrandNode", result.getOrder().getAndIncrement(),
                    new String[]{
                            "productExtInfoNode"
                    });
        };
