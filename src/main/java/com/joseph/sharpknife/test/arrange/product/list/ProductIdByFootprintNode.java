...
@Slf4j
@Component
@TaskConfig(taskName = "productIdByFootprintNode", taskType = "productListType", timeout = 100)
public class ProductIdByFootprintNode implements TaskNode<ProductListContext, ProductListResult> {
    ...
    public BiConsumer<ProductListContext, ProductListResult> getTask() {
        return (context, result) -> {
            ...
            log.info("productIdByFootprintNode task will cost={}", millis);
            ThreadKit.sleep(millis);
            result.finishNode("productIdByFootprintNode", result.getOrder().getAndIncrement(), null);
        };
    }

    @Override
    public void onSuccess(ProductListContext executionContext, ProductListResult executionResult) {
        log.info("productIdByFootprintNode done");
    }

    @Override
    public void onError(ProductListContext executionContext, ProductListResult executionResult, Exception e) {
        log.error("productIdByFootprintNode error", e);
    }
}
