...
@Slf4j
@Component
@DependsOn("oneNodeTimeout2")
@TaskConfig(taskName = "oneNodeTimeout22", taskType = "oneNodeTimeoutType")
public class OneNodeTimeout22 implements TaskNode<SimpleContext, Integer> {


    @Override
    public BiPredicate<SimpleContext, Integer> getPrediction() {
        return (context, result) -> {
            log.info("OneNodeTimeout22 prediction true");
            return true;
        };
    }
    ...
    public BiConsumer<SimpleContext, Integer> getTask() {
        return (context, result) -> {
            ...
            log.info("OneNodeTimeout22 task will cost={}", millis);
