...
@Slf4j
@Component
@DependsOn({"productExtInfoNode", "productInfoNode", "productBusinessLineNode"})
@TaskConfig(taskName = "payRedPacketNode", taskType = "productListType", timeout = 100)
public class PayRedPacketNode implements TaskNode<ProductListContext, ProductListResult> {
    ...
    public BiConsumer<ProductListContext, ProductListResult> getTask() {
        return (context, result) -> {
            ...
            log.info("payRedPacketNode task will cost={}", millis);
            ThreadKit.sleep(millis);
            result.finishNode("payRedPacketNode", result.getOrder().getAndIncrement(),
                    new String[]{
                            "productExtInfoNode",
                            "productInfoNode",
