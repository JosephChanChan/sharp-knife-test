...
@Slf4j
@Component
@DependsOn("userSwitchStatusNode")
@TaskConfig(taskName = "productIdByHomeRecNode", taskType = "productListType", timeout = 200)
public class ProductIdByHomeRecNode implements TaskNode<ProductListContext, ProductListResult> {
    ...
    public BiConsumer<ProductListContext, ProductListResult> getTask() {
        return (context, result) -> {
            int millis = ThreadKit.boundMillis(210);
            log.info("productIdByHomeRecNode task will cost={}", millis);
            ThreadKit.sleep(millis);
            result.finishNode("productIdByHomeRecNode", result.getOrder().getAndIncrement(), new String[]{"userSwitchStatusNode"});
        };
    }

    @Override
    public void onSuccess(ProductListContext executionContext, ProductListResult executionResult) {
        log.info("productIdByHomeRecNode done");
    }

    @Override
    public void onError(ProductListContext executionContext, ProductListResult executionResult, Exception e) {
        log.error("productIdByHomeRecNode error", e);
    }
}
