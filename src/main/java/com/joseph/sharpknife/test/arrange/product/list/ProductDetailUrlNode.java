...
@TaskConfig(taskName = "productDetailUrlNode", taskType = "productListType", timeout = 100)
public class ProductDetailUrlNode implements TaskNode<ProductListContext, ProductListResult> {
    ...
    public BiConsumer<ProductListContext, ProductListResult> getTask() {
        return (context, result) -> {
            ...
            log.info("productDetailUrlNode task will cost={}", millis);
            ThreadKit.sleep(millis);
            result.finishNode("productDetailUrlNode", result.getOrder().getAndIncrement(),
                    new String[]{
                            "productIdAfterNode"
                    });
        };
