...
@Slf4j
@Component
@DependsOn({"productBusinessLineNode", "productIdByFootprintSelloutNode"})
@TaskConfig(taskName = "infoIdToZljProductIdNode", taskType = "productListType", timeout = 100)
public class InfoIdToZljProductIdNode implements TaskNode<ProductListContext, ProductListResult> {
    ...
    public BiConsumer<ProductListContext, ProductListResult> getTask() {
        return (context, result) -> {
            ...
            log.info("infoIdToZljProductIdNode task will cost={}", millis);
            ThreadKit.sleep(millis);
            result.finishNode("infoIdToZljProductIdNode", result.getOrder().getAndIncrement(),
                    new String[]{
                            "productBusinessLineNode",
                            "productIdByFootprintSelloutNode"
                    });
        };
    }
